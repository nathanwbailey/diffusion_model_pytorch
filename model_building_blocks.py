...
class ResidualBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        self.downsample_layer = torch.nn.Conv2d(
...
class DownBlock(torch.nn.Module):
    ...
    def __init__(
        self, block_depth: int, in_channels: int, out_channels: int
    ) -> None:
        """Init Variables and layers."""
        super().__init__()
        layer_list = [
...
class UpBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        layer_list = [
            ResidualBlock(
                in_channels=in_channels + skip_size,
                out_channels=out_channels,
                kernel_size=3,
                padding=1,
                stride=1,
            ...
            ResidualBlock(
                in_channels=out_channels + skip_size,
                out_channels=out_channels,
                kernel_size=3,
                padding=1,
                stride=1,
    ...
    def forward(
        self, input_tensor: torch.Tensor, skips: list[torch.Tensor]
    ) -> torch.Tensor:
